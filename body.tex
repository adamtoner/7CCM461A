...
Cox \fcite{cox72} introduced what he referred to as a conditional likelihood which was a likelihood function conditioned on the set ${t_i}$ of instants at which failure occurs. The reasoning for this is there is no information about the hazard other than the times at which and multiplicities with which failure occurs. 
...
The log-likelihood likelihood function for the model conditioned on ${t_i}$ is then given by taking the natural logarithm of the above function summing over all events as follows:
