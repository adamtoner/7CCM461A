...
\subsection{Tied Failure Times}

Even if just considering the partial likelihood and ignoring the likelihood of the nuisance parameter for now, problems still arise in discrete time when failure times are tied. Cox \fcite{cox72} states that a relatively \emph{ad hoc} modification of the partial likelihood can be made. For a larger proportion of ties, a generalisation is required.

\subsubsection{Cox Method}

Cox \fcite{cox72} proposed a modification to the partial likelihood formula to account for tied failure times by considering the logistic model. The extension is as follows

\begin{align}
    L(\Beta;\x)&=\prod_{i=1}^kpr(j_{i1},\ldots,j_{id_i};d_i,\R_i)\\
    &=\prod_{i=1}^k\frac{pr(j_i1,\ldots,j_{id_i})}{\sum_{l\in\R(t_{(i)};m_{(i)})} pr(l_1,\ldots,l_{d_i})}\\
    &=\prod_{i=1}^k\frac{\exp(\s_i\Beta)}{\sum_{l\in\R(t_{(i)};m_{(i)})}\exp(\s_l\Beta)},
\end{align}

where $\s_(i)=\sum_{j=1}^{m_{(i)}}\z_i$ and $\R(t_{(i)};m_{(i)})$ is the set of all subsets of size $m_{(i)}$ drawn from $\R(t_{(i)})$. The partial log-likelihood is then given by

\begin{equation}
    l(\Beta;\x)=\sum_{i=1}^k\s_i\Beta-\sum_{i=1}^k\log\Big(\sum_{l\in\R(t_{(i)};m_{(i)})}\exp(\s_l\Beta)\Big).
\end{equation}
...
This is another exact method of calculating the partial likelihood in the event of tied times instroduced by Kalbfeisch and Prentice \fcite{kalbfleisch-prentice-73}. The method assumes that tied times occur due to imprecision in measurement and that there must be a true ordering of the events. The probability of each possible is ordering and summed to give the likelihood at time $t_{(i)}$ as

\begin{equation}
    L(\Beta;\x)=\prod_{i=1}^k\frac{\exp(\s_{(i)}\Beta)}{\sum_{(p_1,\ldots,p_{m_{(i)}})\in Q_i}\prod_{j=1}^{m_{(i)}}\sum_{l\in\R(t_{(i)},p_j)}\exp(\z_l\Beta)},
\end{equation}
...
\begin{equation}
    L(\Beta;\x)\approx\prod_{i=1}^k\frac{\exp(\s_i\Beta)}{\big[\sum_{l\in\R(t_{(i)})}\exp(\z_l\Beta)\big]^{m_{(i)}}}
\end{equation}
